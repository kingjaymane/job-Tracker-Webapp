...
export function EmailIntegrationProvider({ children }: EmailIntegrationProviderProps) {
  ...
  const scanEmails = async () => {
    ...
    try {
      ...
      console.log('=== EMAIL SCAN RESULTS ===', {
        totalJobsFound: scanResult.jobApplications.length,
        jobDetails: scanResult.jobApplications.map(job => ({
          company: job.company,
          title: job.jobTitle,
          messageId: job.messageId,
          date: job.date?.toISOString().split('T')[0] || 'unknown',
          confidence: job.confidence,
          subject: job.emailSubject?.substring(0, 50) || 'no subject'
        }))
      });
      
      // Get existing jobs once to avoid repeated queries
      ...
      console.log('=== EXISTING DATABASE JOBS ===', {
        totalExistingJobs: existingJobs.length,
        emailImportedJobs: existingJobs.filter(job => !!job.emailMessageId).length,
        existingMessageIds: Array.from(existingEmailMessageIds)
      });
      ...
      for (const jobApp of scanResult.jobApplications) {
        try {
          ...
          const alreadyExists = existingEmailMessageIds.has(jobApp.messageId);
          
          console.log(`Processing job: ${jobApp.company} - ${jobApp.jobTitle}`, {
            messageId: jobApp.messageId,
            alreadyExists: alreadyExists,
            willBeAdded: !alreadyExists
          });
          ...
          if (!alreadyExists) {
            ...
            console.log(`Skipping existing job: ${jobApp.company} - ${jobApp.jobTitle}`, {
              messageId: jobApp.messageId,
      ...
      console.log('=== FINAL SCAN RESULTS ===', {
        totalJobsFound: scanResult.jobApplications.length,
        jobsAlreadyExisted: scanResult.jobApplications.length - newJobsAdded,
        newJobsAdded: newJobsAdded,
        allNewJobsWillHaveNewFlag: true
      });
      ...
      if (newJobsAdded > 0) {
        toast({
          title: `✨ ${newJobsAdded} New Job${newJobsAdded === 1 ? '' : 's'} Found!`,
          description: `Successfully added ${newJobsAdded} new application${newJobsAdded === 1 ? '' : 's'} from ${scanResult.jobEmailsFound} job-related emails.`,
          duration: 6000,
        });
      } else if (scanResult.jobEmailsFound > 0) {
        toast({
          title: "Scan Complete",
          description: `Found ${scanResult.jobEmailsFound} job-related emails, but no new applications to add. All emails were already imported.`,
          duration: 4000,
        });
      } else {
        toast({
          title: "Scan Complete",
          description: `Scanned ${scanResult.totalEmails} emails but found no new job-related content.`,
          duration: 3000,
        });
      }
