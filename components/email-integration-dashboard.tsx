...
export function EmailIntegrationDashboard({ className, onJobsUpdated }: EmailIntegrationDashboardProps) {
  ...
  const handleScanEmails = async () => {
    try {
      await scanEmails();
      // After scanning is complete, wait a moment for database operations to finish
      // then refresh the job list on the main page
      if (onJobsUpdated) {
        console.log('Email scan completed, refreshing job list...');
        setTimeout(async () => {
          await onJobsUpdated();
          console.log('Job list refreshed after email scan');
