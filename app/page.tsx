...
import { JobRecategorization } from '@/components/job-recategorization';
...
export default function Home() {
  ...
  // Track if this is the initial load to prevent showing notifications on page load
  ...
  const loadJobs = useCallback(async () => {
    ...
    try {
      ...
      // Check for new jobs and show notification (but not on initial load)
      if (!isInitialLoad) {
        const newJobs = jobsData.filter(job => job.isNew);
        if (newJobs.length > 0) {
          // Small delay to ensure the toast shows after loading is complete
          setTimeout(() => {
            toast({
              title: `${newJobs.length} New Job${newJobs.length === 1 ? '' : 's'} Added!`,
              description: newJobs.length === 1 
                ? `${newJobs[0].jobTitle} at ${newJobs[0].companyName}`
                : `${newJobs.map(job => `${job.companyName}`).join(', ')} and more`,
              duration: 5000,
            });
          }, 100);
        }
      ...
      // Mark that initial load is complete even if it failed
      if (isInitialLoad) {
        setIsInitialLoad(false);
      }
    } finally {
  ...
  const addJob = async (job: Omit<JobApplication, 'id'>) => {
    ...
    try {
      ...
      // Show success notification for manually added jobs
      toast({
        title: "Job Added Successfully!",
        description: `${job.jobTitle} at ${job.companyName} has been added to your tracker.`,
        duration: 4000,
      });
    } catch (error) {
      ...
      toast({
        ...
        description: "Failed to add the job. Please try again.",
        variant: "destructive",
      });
    }
  };
    ...
    try {
      ...
      if (job) {
        toast({
          ...
          description: `${job.jobTitle} at ${job.companyName} is now marked as ${status}.`,
          duration: 3000,
        });
      }
    } catch (error) {
  ...
  const deleteJob = async (id: string) => {
    ...
    try {
      ...
      if (job) {
        toast({
          title: "Job Deleted",
          description: `${job.jobTitle} at ${job.companyName} has been removed.`,
          duration: 3000,
        });
      }
    } catch (error) {
  ...
  const editJob = async (id: string, updates: Partial<JobApplication>) => {
    ...
    try {
      ...
      if (job) {
        toast({
          title: "Job Updated",
          description: `${job.jobTitle} at ${job.companyName} has been updated successfully.`,
          duration: 3000,
        });
      }
    } catch (error) {
            ...
            {currentView === 'settings' && (
              <div className="p-6">
                <div className="max-w-4xl mx-auto space-y-6">
                  ...
                  <JobRecategorization onJobsUpdated={loadJobs} />
